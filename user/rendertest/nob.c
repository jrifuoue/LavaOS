...
int main(int argc, char **argv) {
    ...
    if(!nob_mkdir_if_not_exists_silent(temp_sprintf("%s/rendertest", bindir))) return 1;
    const char* output = temp_sprintf("%s/rendertest/rendertest", bindir);
    File_Paths pathb = { 0 };
    ...
    if(rootdir && !copy_file(output, temp_sprintf("%s/user/rendertest", rootdir))) 
        return 1;
}
